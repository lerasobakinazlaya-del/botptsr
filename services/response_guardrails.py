...
def _compress_risky_scene_lecture(text: str, *, user_message: str = "") -> str:
    ...
    rave_markers = ("\u0440\u0435\u0439\u0432", "\u0431\u0435\u0440\u043b\u0438\u043d", "berlin")
    has_rave = any(marker in request for marker in rave_markers)
    has_group_scene = any(
        marker in request
        for marker in ("\u043e\u0440\u0433", "\u0432\u0442\u0440\u043e\u0435\u043c", "\u0432\u0447\u0435\u0442\u0432\u0435\u0440\u043e\u043c", "\u043c\u0436\u043c", "\u0436\u043c\u0436", "\u043c\u043c\u0436")
    )

    if has_drugs and has_rave:
        return (
            "Я на это смотрю так: сам рейв может быть ок, но смесь «наркотики + групповой секс + незнакомая сцена» "
            "чаще ломает не запрет, а твои собственные ощущения. Вместо кайфа люди нередко получают мутность, "
            "потерю темпа и утро, которое вообще было не про то."
        )

    if has_drugs and has_no_barrier:
        return (
            "Я бы на это смотрел без романтизации: ты складываешь в одну точку сразу несколько вещей, "
            "которые плохо дружат между собой. Под кайфом и без защиты такая ночь чаще уезжает из "
            "\"хочу попробовать\" в обычный бардак с плохими последствиями."
        )

    if has_drugs:
        return (
            "Я бы на это смотрел без романтизации: под веществами такая история чаще не усиливает впечатление, "
            "а сбивает восприятие и контроль. В голове это выглядит ярче, чем потом ощущается вживую."
        )

    if has_group_scene:
        return (
            "Тут всё очень быстро срывается не потому, что кто-то плохой, а потому что людей и ожиданий сразу много. "
            "Если у нескольких человек в голове разные версии этой ночи, возбуждение кончается раньше, чем начинается "
            "нормальный контакт."
        )

    return (
        "Я бы на это смотрел трезво: такие сцены легко разваливаются, когда в голове одна картинка, "
        "а в реальности совсем другой темп и другие границы. Поэтому это не та история, где стоит надеяться, "
        "что всё само красиво сложится."
    )
...
def _compress_charged_probe_lecture(text: str, *, user_message: str = "") -> str:
    ...
    if any(marker in request for marker in ("втроем", "тройнич", "группов", "оргия", "секс", "жмж", "мжм", "ммж", "мжмж")):
        if any(marker in request for marker in ("организ", "стоит ли", "как", "расскажи", "помоги", "план")):
            return (
                "Тогда это лучше разбирать не как фантазию, а как реальный разговор между людьми. "
                "Сначала отделите желание от решения: кто правда хочет участвовать, что для каждого точно ок, что табу, "
                "как останавливаете процесс и что делаете, если кому-то стало не по себе."
            )
        if "хочу" in request:
            return (
                "Ок, желание понятно. Тут важнее не сама картинка, а насколько все участники правда хотят одного и того же "
                "и могут спокойно остановиться в любой момент."
            )
        return (
            "Тут обычно всё держится не на схеме, а на договорённости между людьми: желание, границы, темп и право выйти из ситуации."
        )
    return (
...
def _deplatitudize_comfort_response(text: str, *, user_message: str = "") -> str:
    normalized = " ".join(str(text or "").split()).strip()
    if not normalized:
        return normalized
    ...
    low_value_markers = (
        ...
        "\u043f\u043e\u0438\u0441\u043a\u043e\u043c \u043d\u043e\u0432\u043e\u0439 \u0440\u0430\u0431\u043e\u0442\u044b \u0438\u043b\u0438 \u0445\u043e\u0431\u0431\u0438",
        "\u044d\u0442\u043e \u043c\u043e\u0436\u0435\u0442 \u043e\u0442\u0432\u043b\u0435\u0447\u044c",
        "\u043f\u043e\u043c\u043e\u0447\u044c \u0440\u0430\u0441\u0441\u043b\u0430\u0431\u0438\u0442\u044c\u0441\u044f",
        "\u043d\u0430\u0439\u0442\u0438 \u0445\u043e\u0431\u0431\u0438",
    )
    if not any(marker in lowered for marker in low_value_markers):
        return normalized

    if any(marker in request for marker in ("\u043d\u0430\u0440\u043a\u043e\u0442", "\u0432\u0435\u0449\u0435\u0441\u0442\u0432", "\u0437\u0430\u0432\u0438\u0441\u0438\u043c", "\u043c\u0435\u0444")):
        return (
            "\u0422\u0443\u0442 \u043d\u0435 \u043f\u0440\u043e \u043a\u0440\u0430\u0441\u0438\u0432\u0443\u044e \u0441\u0430\u043c\u043e\u043e\u043f\u0442\u0438\u043c\u0438\u0437\u0430\u0446\u0438\u044e \u0438 \u043d\u0435 \u043f\u0440\u043e \u0434\u0435\u043a\u043e\u0440\u0430\u0442\u0438\u0432\u043d\u0443\u044e \u043f\u0430\u0443\u0437\u0443. "
            "\u041a\u043e\u0433\u0434\u0430 \u0440\u0430\u0431\u043e\u0442\u044b \u043d\u0435\u0442, \u0430 \u0432\u0435\u0449\u0435\u0441\u0442\u0432\u0430 \u0440\u044f\u0434\u043e\u043c, \u0447\u0435\u043b\u043e\u0432\u0435\u043a\u0430 \u043e\u0431\u044b\u0447\u043d\u043e \u0442\u0430\u0449\u0438\u0442 \u043d\u0435 \u043f\u043e\u0442\u043e\u043c\u0443, \u0447\u0442\u043e \u043e\u043d \u043b\u0435\u043d\u0438\u0432\u044b\u0439, \u0430 \u043f\u043e\u0442\u043e\u043c\u0443 \u0447\u0442\u043e \u043e\u043f\u043e\u0440\u044b \u043c\u0430\u043b\u043e. "
            "\u042f \u0431\u044b \u0441\u0435\u0439\u0447\u0430\u0441 \u0441\u043c\u043e\u0442\u0440\u0435\u043b \u043d\u0435 \u043d\u0430 \u0431\u043e\u043b\u044c\u0448\u0438\u0435 \u043e\u0431\u0435\u0449\u0430\u043d\u0438\u044f, \u0430 \u043d\u0430 \u043e\u0434\u0438\u043d \u0440\u0435\u0430\u043b\u044c\u043d\u044b\u0439 \u0448\u0430\u0433, \u043a\u043e\u0442\u043e\u0440\u044b\u0439 \u0443\u043c\u0435\u043d\u044c\u0448\u0438\u0442 \u0441\u043d\u043e\u0441 \u0441\u0435\u0433\u043e\u0434\u043d\u044f."
        )

    if "\u0440\u0430\u0431\u043e\u0442" in request:
        return (
            "\u041f\u043e\u0445\u043e\u0436\u0435, \u0442\u0435\u0431\u044f \u0441\u0435\u0439\u0447\u0430\u0441 \u043f\u0440\u0438\u0436\u0438\u043c\u0430\u0435\u0442 \u043d\u0435 \u0430\u0431\u0441\u0442\u0440\u0430\u043a\u0442\u043d\u0430\u044f \u0443\u0441\u0442\u0430\u043b\u043e\u0441\u0442\u044c, \u0430 \u0441\u0430\u043c\u0430 \u043f\u043e\u0442\u0435\u0440\u044f \u043e\u043f\u043e\u0440\u044b \u0432\u043e\u043a\u0440\u0443\u0433 \u0440\u0430\u0431\u043e\u0442\u044b. "
            "\u0421\u043e\u0432\u0435\u0442 \u0432 \u0434\u0443\u0445\u0435 \u00ab\u043e\u0442\u0432\u043b\u0435\u043a\u0438\u0441\u044c\u00bb \u0442\u0443\u0442 \u043e\u0431\u044b\u0447\u043d\u043e \u0442\u043e\u043b\u044c\u043a\u043e \u0431\u0435\u0441\u0438\u0442. "
            "\u041b\u0443\u0447\u0448\u0435 \u0441\u043d\u0430\u0447\u0430\u043b\u0430 \u043d\u0430\u0437\u0432\u0430\u0442\u044c, \u0447\u0442\u043e \u0438\u043c\u0435\u043d\u043d\u043e \u0440\u0430\u0437\u0432\u0430\u043b\u0438\u043b\u043e\u0441\u044c: \u0434\u0435\u043d\u044c\u0433\u0438, \u0440\u0438\u0442\u043c, \u0441\u0430\u043c\u043e\u043e\u0446\u0435\u043d\u043a\u0430 \u0438\u043b\u0438 \u043e\u0449\u0443\u0449\u0435\u043d\u0438\u0435 \u0431\u0443\u0434\u0443\u0449\u0435\u0433\u043e."
        )

    return (
        "\u0422\u0443\u0442 \u043b\u0443\u0447\u0448\u0435 \u043d\u0435 \u0434\u0435\u043b\u0430\u0442\u044c \u0432\u0438\u0434, \u0447\u0442\u043e \u0432\u0441\u0451 \u0447\u0438\u043d\u0438\u0442\u0441\u044f \u043f\u0430\u0443\u0437\u043e\u0439 \u0438\u043b\u0438 \u043f\u043e\u043b\u0435\u0437\u043d\u044b\u043c \u0437\u0430\u043d\u044f\u0442\u0438\u0435\u043c. "
        "\u0415\u0441\u043b\u0438 \u0442\u0435\u0431\u044f \u043f\u0440\u0430\u0432\u0434\u0430 \u043f\u0440\u0438\u0434\u0430\u0432\u0438\u043b\u043e, \u0432\u0430\u0436\u043d\u0435\u0435 \u0441\u043d\u0430\u0447\u0430\u043b\u0430 \u0432\u0435\u0440\u043d\u0443\u0442\u044c \u043d\u0435\u043c\u043d\u043e\u0433\u043e \u043e\u043f\u043e\u0440\u044b, \u0430 \u043d\u0435 \u043f\u0440\u0438\u0434\u0443\u043c\u044b\u0432\u0430\u0442\u044c \u043f\u0440\u0430\u0432\u0438\u043b\u044c\u043d\u044b\u0439 \u043e\u0431\u0440\u0430\u0437 \u0436\u0438\u0437\u043d\u0438."
    )
...
def _build_dialogue_pull_question(user_message: str) -> str:
    ...
    if any(marker in request for marker in ("орг", "втроем", "вчетвером", "секс", "жмж", "мжм", "ммж", "мжмж", "тройнич", "группов")):
        if any(marker in request for marker in ("организ", "стоит ли", "как", "расскажи", "помоги", "план")):
            return "Вы сейчас обсуждаете это как фантазию или как реальный шаг, который хотите сделать?"
        return "Это для тебя пока фантазия, разговор с партнёрами или уже реальный план?"
    if topic == "pricing":
...
def apply_human_style_guardrails(
    ...
    *,
    ...
    if active_mode == "comfort":
        guarded = _deplatitudize_comfort_response(guarded, user_message=user_message)
