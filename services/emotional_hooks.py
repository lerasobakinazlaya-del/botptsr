...
HOOK_LIBRARY: dict[str, tuple[str, ...]] = {
    ...
    "escalation_hooks": (
        ...
        "и если туда пойти, динамика быстро станет плотнее",
        "но именно после этого обычно начинается самое важное",
    ),
...
def ensure_open_loop(text: str) -> str:
    ...
    if not body:
        return normalized
    return f"{body}, и дальше все держится на одной детали."
