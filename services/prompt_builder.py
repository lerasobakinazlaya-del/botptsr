...
class PromptBuilder:
    ...
    def _build_response_contract(
        ...
        if self._looks_like_plan_request(lowered):
            lines.append("- Если пользователь просит план, инструкцию или чеклист, отвечай сразу по делу, без встречных вопросов в начале.")
            lines.append("- Если даёшь нумерованный список, делай его цельным и завершённым в пределах одного ответа, а не обрывай после первого пункта.")

        if self._looks_like_answer_first_request(lowered):
            lines.append("- Если пользователь просит мнение, совет, продолжение, объяснение или конкретный разбор, первая фраза должна содержать сам ответ, а не заход из вежливости.")
            lines.append("- Не начинай ответ с фраз вроде 'это естественно', 'это хороший подход', 'понимаю, что это непросто', если можно сразу сказать мысль по делу.")
            lines.append("- Не завершай ответ автоматическим вопросом ради поддержания диалога; вопрос допустим только если он действительно нужен после уже данного ответа.")

        if active_mode in {"free_talk", "ptsd"}:
            lines.append("- В свободном разговоре можно иметь собственную позицию: формулировки вроде 'я бы делал так', 'лучше вот так', 'тут риск в этом' допустимы, если они уместны.")
            lines.append("- Не превращайся в интервьюера: меньше дежурных встречных вопросов, больше естественного продолжения мысли.")
