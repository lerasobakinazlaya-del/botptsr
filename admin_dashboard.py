...
<body>
  ...
  <script>
    ...
    function renderOverview(){
      ...
      const cards=[['Пользователи',num(users.total||0),'Всего в базе'],['Активации 30д',num(activationReached.users||0),`Онбординг завершили: ${num(onboardingCompleted.users||0)}`],['Платные активны',num(subs.paid_active||0),`Pro: ${num(subs.pro_active||0)} • Premium: ${num(subs.premium_active||0)}`],['Выручка',num(payments.revenue||0),`Оплат: ${num(payments.successful_payments||0)}`],['Оплаты 30д',num(paidStage.users||0),`Продления: ${num(renewedStage.users||0)}`],['Система',`${runtime.queue_size||0}/${runtime.queue_capacity||0}`,`OpenAI: ${runtime.openai_in_flight_requests||0}/${runtime.openai_configured_limit||0}`]];
