...
async def lifespan(app: FastAPI):
    ...
    container.admin_metrics = AdminMetricsService(
        ...
        openai_usage_repository=container.openai_usage_repository,
...
def _build_channel_calendar_preview() -> dict[str, Any]:
    ...
    for raw_item in sorted(schedule.get("items", []), key=lambda item: str(item.get("publish_at") or "")):
        ...
        preview_image_file = Path(str(item.get("preview_image_file") or "")) if item.get("preview_image_file") else None
        image_file = Path(str(item.get("image_file") or "")) if item.get("image_file") else None
        display_image_file = preview_image_file or image_file
        image_data_url = ""
        if display_image_file and display_image_file.exists():
            image_data_url = "data:image/png;base64," + base64.b64encode(display_image_file.read_bytes()).decode("ascii")
        ...
        items.append(
            {
                ...
                "preview_image_file": preview_image_file.as_posix() if preview_image_file else "",
...
<head>
  ...
  <style>
    :root{--bg:#f4f6f8;--panel:#fff;--soft:#f7f9fb;--text:#17212b;--muted:#66758a;--accent:#2563eb;--accent-2:#10b981;--warn:#b7791f;--danger:#c2413d;--border:#d9e0ea;--nav:#0f172a;--nav-soft:#111c33;--shadow:0 8px 24px rgba(15,23,42,.08);--content-max:1600px}
    *{box-sizing:border-box}body{margin:0;color:var(--text);font-family:"Segoe UI",Arial,sans-serif;background:var(--bg)}
    .layout{display:grid;grid-template-columns:292px minmax(0,1fr);min-height:100vh}.sidebar{padding:18px 14px;border-right:1px solid var(--border);background:var(--nav);color:#eef4ff}.sidebar-inner{position:sticky;top:18px;display:grid;gap:16px;max-height:calc(100vh - 36px);overflow:auto;padding-right:2px}.main{padding:20px;display:grid;gap:14px;min-width:0}.main>*{width:100%;max-width:var(--content-max);margin:0 auto}
    .brand-card{padding:18px;border:1px solid rgba(255,255,255,.12);border-radius:8px;background:var(--nav-soft);box-shadow:var(--shadow)}.brand-eyebrow{font-size:11px;text-transform:uppercase;letter-spacing:.14em;color:#93c5fd;margin-bottom:10px}.brand-title{margin:0 0 10px;font-size:28px;line-height:1.05}.brand-copy{margin:0;color:#b6c3d6;line-height:1.5}
    .nav{display:grid;gap:14px}.nav-group{display:grid;gap:6px}.nav-label{font-size:11px;text-transform:uppercase;letter-spacing:.12em;color:#8ea3c1;padding:0 4px}.nav button,.toolbar button,.actions button{border:1px solid var(--border);background:var(--soft);color:var(--text);border-radius:8px;padding:10px 13px;cursor:pointer;font-weight:600;transition:background .15s ease,border-color .15s ease,box-shadow .15s ease}.nav button{background:transparent;border-color:rgba(255,255,255,.1);color:#dbe7f7;text-align:left;width:100%}.nav button:hover,.toolbar button:hover,.actions button:hover{border-color:#9bb6da;box-shadow:0 4px 14px rgba(37,99,235,.08)}.nav button.active,.toolbar .primary,.actions .primary{background:var(--accent);color:#fff;border-color:var(--accent);box-shadow:0 8px 18px rgba(37,99,235,.2)}
    ...
    .toolbar{display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin-bottom:14px}.toolbar input,.toolbar select{flex:1 1 220px;min-width:0}.toolbar button{flex:0 0 auto}.toolbar-context{font-weight:700;margin-right:auto}.toolbar-context .muted{font-size:12px;font-weight:500}.top-toolbar{position:sticky;top:12px;z-index:3;padding:12px 14px;margin-bottom:0;border:1px solid var(--border);border-radius:8px;background:rgba(255,255,255,.94);backdrop-filter:blur(12px);box-shadow:var(--shadow)}
...
<body>
  <div class="layout">
    <aside class="sidebar">
      <div class="sidebar-inner">
        ...
        <div class="nav">
          <div class="nav-group">
            <div class="nav-label">Операции</div>
          ...
          <div class="nav-group">
            <div class="nav-label">Клиенты</div>
          ...
          <div class="nav-group">
            <div class="nav-label">Продукт</div>
    ...
    <main class="main">
      <div class="toolbar top-toolbar">
        <div class="toolbar-context">
          <div id="toolbar-title">Обзор продукта</div>
          <div class="muted" id="toolbar-subtitle">Операционный центр</div>
        </div>
      ...
      <section class="page active" data-view="overview">
        ...
        <div class="panel">
          <div class="mode-head">
            <div>
              <h3>Расход OpenAI</h3>
              <p class="muted section-note">Отдельный блок по токенам, пользователям и стоимости, чтобы не искать расход по нескольким карточкам.</p>
            </div>
            <button data-open-view="health">Открыть health</button>
          </div>
          <div id="overview-openai-spend"></div>
        </div>
        <div class="three">
      ...
      <section class="page" data-view="setup">
        ...
        <div class="cols">
          <div class="panel">
            <h3>OpenAI usage alerts</h3>
            <p class="muted section-note">Пороги расхода и быстрый статус по токенам, вызовам и источникам за последние 24 часа.</p>
            <div class="soft-panel">
              <label class="checkbox"><input id="usage_alerts_enabled" type="checkbox">Включить алерты расхода</label>
            </div>
            <div class="three">
              <label>Токены warn / 24ч<input id="usage_alerts_daily_tokens_warn" type="number" min="0" step="1"></label>
              <label>Токены high / 24ч<input id="usage_alerts_daily_tokens_high" type="number" min="0" step="1"></label>
              <label>Вызовы warn / 24ч<input id="usage_alerts_daily_requests_warn" type="number" min="0" step="1"></label>
              <label>Source tokens warn<input id="usage_alerts_source_daily_tokens_warn" type="number" min="0" step="1"></label>
              <label>Source requests warn<input id="usage_alerts_source_daily_requests_warn" type="number" min="0" step="1"></label>
              <label>Source share warn, %<input id="usage_alerts_source_share_warn_pct" type="number" min="0" max="100" step="1"></label>
            </div>
            <label>Исключённые sources<textarea id="usage_alerts_excluded_sources" placeholder="reengagement&#10;&#10;admin_test_reply"></textarea></label>
          </div>
          <div class="panel">
            <h3>Статус OpenAI-расхода</h3>
            <p class="muted section-note">Живой срез usage из health, чтобы сразу увидеть, кто жжёт токены и где уже пробиваются пороги.</p>
            <div id="runtime-openai-alert-status" class="stack"><div class="muted">Пока нет данных.</div></div>
          </div>
        </div>
        <div class="panel">
        ...
        <div class="cols">
          ...
          <div class="panel">
            ...
            <div class="soft-panel">
              ...
              <label class="checkbox"><input id="limits_free_monthly_messages_enabled" type="checkbox">Включить месячный лимит сообщений для бесплатных пользователей</label>
              <label class="checkbox"><input id="limits_free_monthly_tokens_enabled" type="checkbox">Включить месячный лимит chat-токенов для бесплатных пользователей</label>
            ...
            <div class="two">
              ...
              <label>Лимит бесплатных сообщений в месяц<input id="limits_free_monthly_messages_limit" type="number" min="1"></label>
              <label>Лимит бесплатных chat-токенов в месяц<input id="limits_free_monthly_tokens_limit" type="number" min="1"></label>
              ...
              <label>Текст при исчерпании месячного лимита сообщений<textarea id="limits_free_monthly_limit_message"></textarea></label>
              <label>Текст при исчерпании месячного лимита токенов<textarea id="limits_free_monthly_tokens_limit_message"></textarea></label>
  ...
  <script>
    ...
    const state={
      ...
      currentUser:{},
      currentConversation:{user:{},messages:[],long_term_memories:[],state:{},profile:{}},
      currentMemoryId:null,
      selectedUserIds:new Set(),
      lastBroadcastPreview:null,
    ...
    function currentMessageTemplates(){const runtime=((state.settings||{}).runtime||{}),ui=runtime.ui||{},templates=ui.message_templates;return Array.isArray(templates)&&templates.length?templates:DEFAULT_MESSAGE_TEMPLATES}
    ...
    function renderOpenAIAlertStatus(usage,alerts){
      ...
      sourceRows.forEach(([source,stats])=>{
        ...
        if(overTokens||overRequests||overShare)issues.push(`${source}: ${num(tokens)} токенов, ${num(requests)} вызовов, ${share}% дня.`);
      });
      const summary=metricCards([
        ['Токены 24ч',num(data.tokens_1d||0),`warn ${num(cfg.daily_tokens_warn||0)} • high ${num(cfg.daily_tokens_high||0)}`],
    ...
    function renderChrome(){
      ...
      setText('#toolbar-title',meta.title);
      setText('#toolbar-subtitle',meta.kicker);
    ...
    function renderOverview(){
      ...
      const activationReached=growthEvents.activation_reached||{},onboardingCompleted=growthEvents.onboarding_completed||{},onboardingStarted=growthEvents.onboarding_started||{},referralMenuOpened=growthEvents.referral_menu_opened||{},insightShared=growthEvents.insight_shared||{},acquisitionAttributed=growthEvents.acquisition_attributed||{},subs=users.subscription_segments||{},paidStage=funnel30Stages.paid||{},renewedStage=funnel30Stages.renewed||{},offerShownStage=funnel30Stages.offer_shown||{},invoiceOpenedStage30=funnel30Stages.invoice_opened||{},offerShownStage7=funnel7Stages.offer_shown||{},invoiceOpenedStage7=funnel7Stages.invoice_opened||{},paidStage7=funnel7Stages.paid||{},renewedStage7=funnel7Stages.renewed||{};
      ...
      const cards=[['Пользователи',num(users.total||0),'Всего в базе'],['Активации 30д',num(activationReached.users||0),`Онбординг завершили: ${num(onboardingCompleted.users||0)}`],['Платные активны',num(subs.paid_active||0),`Pro: ${num(subs.pro_active||0)} • Premium: ${num(subs.premium_active||0)}`],['Выручка',num(payments.revenue||0),`Оплат: ${num(payments.successful_payments||0)}`],['OpenAI 1д',num(aiUsage.tokens_1d||0),`Запросов: ${num(aiUsage.requests_1d||0)}`],['Система',`${runtime.queue_size||0}/${runtime.queue_capacity||0}`,`OpenAI: ${runtime.openai_in_flight_requests||0}/${runtime.openai_configured_limit||0}`]];
      ...
      $('#overview-openai-spend').innerHTML=`<div class="stack">${metricCards([['Токены 24ч',num(aiUsage.tokens_1d||0),`Запросов: ${num(aiUsage.requests_1d||0)}`],['Токены 7д',num(aiUsage.tokens_7d||0),`Пользователей: ${num(aiUsage.users_7d||0)}`],['Токены всего',num(aiUsage.tokens_total||0),`Вызовов: ${num(aiUsage.requests_total||0)}`],['Prompt tokens',num(aiUsage.prompt_tokens_total||0),'Входные токены'],['Completion tokens',num(aiUsage.completion_tokens_total||0),'Выходные токены'],['Стоимость',aiUsage.estimated_cost_usd_total!=null?String(aiUsage.estimated_cost_usd_total):'0',`USD всего • 24ч: ${aiUsage.estimated_cost_usd_1d!=null?aiUsage.estimated_cost_usd_1d:0}`]])}${kvList([['Пользователей с расходом',esc(num(aiUsage.users_total||0))],['Средняя задержка OpenAI',esc(`${aiUsage.avg_latency_ms||0} мс`)],['Максимальная задержка OpenAI',esc(`${aiUsage.max_latency_ms||0} мс`)],['Модельный топ за 30д',esc(Object.keys(aiUsage.by_model_30d||{})[0]||'—')],['Главный source за 1д',esc(Object.keys(aiUsage.by_source_1d||{})[0]||'—')],['Алерты расхода',esc((state.health&&state.health.warnings&&state.health.warnings.length)||0)]])}</div>`;
      ...
      $('#support-summary').innerHTML=`<div class="stack">${metricCards([['Профили поддержки',String(support.users_with_support_profile||0),'Пользователи с профилем поддержки'],['Рефералы',String(referrals.total||0),`Конверсий: ${referrals.converted||0}`],['Инициативные пользователи',String(proactive.users_contacted_7d||0),'Кому бот писал за 7 дней'],['Неуспешные инициативы',String(proactive.failed_total||0),`За 1 день: ${proactive.failed_1d||0}`]])}${kvList([['Эпизоды паники',esc(num(episodes.panic||0))],['Эпизоды флэшбэков',esc(num(episodes.flashback||0))],['Эпизоды бессонницы',esc(num(episodes.insomnia||0))],['Флаги самоповреждения',esc(num(support.self_harm_flags||0))],['Отправлено инициативных',esc(num(proactive.sent_total||0))],['Не отправлено',esc(num((proactive.status_breakdown_total||{}).failed||0))],['Заблокировано',esc(num((proactive.status_breakdown_total||{}).blocked||0))],['Ошибка отправки',esc(num((proactive.status_breakdown_total||{}).send_failed||0))],['Не сохранилось в БД',esc(num((proactive.status_breakdown_total||{}).persist_failed||0))],['Ответы после инициативных',esc(`${proactive.reply_after_proactive_total||0} (${proactive.reply_after_proactive_rate||0}%)`)],['OpenAI всего',esc(`${num(aiUsage.tokens_total||0)} токенов / ${num(aiUsage.requests_total||0)} вызовов`)],['Пользователи с часовым поясом',esc(num(preferences.users_with_timezone||0))],['Пользователи с отказом',esc(num(preferences.proactive_disabled_users||0))],['Последнее обновление',esc(support.last_updated_at||'Нет данных')]])}</div>`;
      const proactiveStatusRows=Object.entries(proactive.status_breakdown_total||{}).map(entry=>({Статус:proactiveStatusLabel(entry[0]),Код:String(entry[0]||'unknown'),'Всего':num(entry[1]||0),'7 дней':num(((proactive.status_breakdown_7d||{})[entry[0]])||0)}));
      const proactiveFailureRows=(proactive.recent_failures||[]).map(item=>({'Время':item.created_at||'—','Статус':proactiveStatusLabel(item.status),'Код':item.status||'—','Trigger':item.trigger_kind||'—','User ID':item.user_id||'—','Причина':item.error_text||'—'}));
      $('#proactive-failures').innerHTML=`<div class="stack"><div class="table-wrap overview-table">${table(['Статус','Код','Всего','7 дней'],proactiveStatusRows)}</div><div class="table-wrap overview-table">${table(['Время','Статус','Код','Trigger','User ID','Причина'],proactiveFailureRows)}</div></div>`;
      $('#monetization-summary').innerHTML=`<div class="stack">${metricCards([['Офферы 7д',String(offerShownStage7.users||0),`Инвойс: ${invoiceOpenedStage7.users||0}`],['Оплаты 7д',String(paidStage7.users||0),`Продления: ${renewedStage7.users||0}`],['Онбординг 30д',String(onboardingCompleted.users||0),`Активации: ${activationReached.users||0}`],['Acquisition 30д',String(acquisitionAttributed.users||0),`Share: ${insightShared.events||0}`]])}${kvList([['Конверсия оффер -> инвойс (7д)',esc(`${funnel7Conversion.offer_to_invoice_pct||0}%`)],['Конверсия инвойс -> paid (7д)',esc(`${funnel7Conversion.invoice_to_paid_pct||0}%`)],['Конверсия paid -> renewed (7д)',esc(`${funnel7Conversion.paid_to_renewed_pct||0}%`)],['Events offer_shown / invoice_opened (30д)',esc(`${offerShownStage.events||0} / ${invoiceOpenedStage30.events||0}`)],['Events paid / renewed (30д)',esc(`${paidStage.events||0} / ${renewedStage.events||0}`)],['Top source events',esc(topSourceEntry.length?`${topSourceEntry[0]} • ${topSourceEntry[1].events||0}`:'—')],['Top campaign events',esc(topCampaignEntry.length?`${topCampaignEntry[0]} • ${topCampaignEntry[1].events||0}`:'—')]])}</div>`;
    }
    function renderHealth(){
      if(!state.health)return;
      const db=state.health.db||{},redis=state.health.redis||{},ai=state.health.ai_runtime||{},usage=state.health.openai_usage||{},chat=state.health.chat_runtime||{},release=state.health.release||{},warnings=state.health.warnings||[];
    ...
    function renderRuntime(){
      ...
      const r=state.settings.runtime,a=r.ai||{},c=r.chat||{},e=r.engagement||{},u=r.ui||{},costControl=r.cost_control||{},usageAlerts=costControl.usage_alerts||{},dialogue=a.dialogue||{},fastLane=a.fast_lane||{},style=e.reengagement_style||{},healthUsage=(state.health&&state.health.openai_usage)||{};
      ...
      setChecked('#usage_alerts_enabled',usageAlerts.enabled!==false);
      setValue('#usage_alerts_daily_tokens_warn',usageAlerts.daily_tokens_warn||'');
      setValue('#usage_alerts_daily_tokens_high',usageAlerts.daily_tokens_high||'');
      setValue('#usage_alerts_daily_requests_warn',usageAlerts.daily_requests_warn||'');
      setValue('#usage_alerts_source_daily_tokens_warn',usageAlerts.source_daily_tokens_warn||'');
      setValue('#usage_alerts_source_daily_requests_warn',usageAlerts.source_daily_requests_warn||'');
      setValue('#usage_alerts_source_share_warn_pct',usageAlerts.source_share_warn_pct||'');
      setValue('#usage_alerts_excluded_sources',Array.isArray(usageAlerts.excluded_sources)?usageAlerts.excluded_sources.join('\\n\\n'):'');
      const alertStatus=$('#runtime-openai-alert-status');
      if(alertStatus)alertStatus.innerHTML=renderOpenAIAlertStatus(healthUsage,usageAlerts);
      ...
      setValue('#ui_welcome_followup_text',typeof u.welcome_followup_text==='string'?u.welcome_followup_text:'');
      setValue('#ui_onboarding_prompt_buttons',Array.isArray(u.onboarding_prompt_buttons)?u.onboarding_prompt_buttons.join('\\n'):'');
    ...
    function renderSafety(){
      ...
      $('#limits_free_monthly_messages_enabled').checked=!!l.free_monthly_messages_enabled;
      $('#limits_free_monthly_tokens_enabled').checked=!!l.free_monthly_tokens_enabled;
      ...
      $('#limits_free_monthly_messages_limit').value=l.free_monthly_messages_limit||'';
      $('#limits_free_monthly_tokens_limit').value=l.free_monthly_tokens_limit||'';
      ...
      $('#limits_free_monthly_limit_message').value=l.free_monthly_limit_message||'';
      $('#limits_free_monthly_tokens_limit_message').value=l.free_monthly_tokens_limit_message||'';
    ...
    function renderChannelCalendar(){
      ...
      itemsEl.innerHTML=items.map(item=>{
        ...
        const image=item.image_data_url?`<img src="${item.image_data_url}" alt="${esc(item.id||'post image')}" style="width:100%;max-width:520px;border-radius:18px;border:1px solid var(--border);box-shadow:var(--shadow);margin:8px 0 14px">`:'';
        return `<div class="panel"><div class="mode-head"><div><h3>${esc(item.publish_at_label||'Дата не указана')}</h3><p class="muted section-note">${esc(item.id||'без id')} • ${esc(item.text_file||'без файла')}</p></div><span class="status-pill ${esc(status[0])}">${esc(status[1])}</span></div>${image}<div class="two">${kvList([['Кнопка',button],['Закреп',item.pin?'Да':'Нет'],['Дата ISO',esc(item.publish_at||'—')],['Формат',item.image_file?'текст + чистая картинка':'только текст']])}${kvList([['Статус',esc(item.status||'—')],['Опубликовано',esc(item.published_at||'—')],['Картинка',esc(item.image_file||'—')]])}</div>${published}<h4>Предпросмотр</h4><pre>${esc(item.text||item.excerpt||'Текст пустой.')}</pre></div>`
      }).join('')
    }
    ...
    function qualityRows(result){
      const payload=result||{},response=String(payload.response||''),audit=payload.response_audit||{},prompt=String(payload.prompt||''),notes=[];
      ...
      if(payload.grounding_kind)notes.push(['Заземление', 'warn', `Сработало: ${payload.grounding_kind}`]);
      return notes;
    }
