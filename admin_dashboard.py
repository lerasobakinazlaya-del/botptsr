...
<body>
  ...
  <script>
    ...
    function renderAll(){const renderers=[['overview',renderOverview],['health',renderHealth],['runtime',renderRuntime],['safety',renderSafety],['prompts',renderPrompts],['modes',renderModes],['payments',renderPayments],['logs',renderLogs]];const errors=[];renderers.forEach(([name,fn])=>{try{fn()}catch(error){console.error(`Render failed: ${name}`,error);errors.push(name)}});if(errors.length)notice(`Часть блоков не отрисована: ${errors.join(', ')}`,'error')}
    async function refreshAll(){const requests=[['overview','/api/overview','overview'],['health','/api/health','health'],['settings','/api/settings','settings'],['logs',`/api/logs?lines=${$('#log-lines').value||200}`,'logs']];const results=await Promise.allSettled(requests.map(([,path])=>api(path)));const failed=[];results.forEach((result,index)=>{const [label,,stateKey]=requests[index];if(result.status==='fulfilled'){state[stateKey]=result.value}else{console.error(`Load failed: ${label}`,result.reason);failed.push(label)}});renderAll();if(failed.length)notice(`Не все данные загрузились: ${failed.join(', ')}`,'error')}
