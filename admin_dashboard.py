...
<body>
  ...
  <script>
    ...
    function renderOverview(){if(!state.overview)return;const o=state.overview,users=o.users||{},content=o.content||{},payments=o.payments||{},runtime=o.runtime||{},support=o.support||{},episodes=support.episode_counts||{},proactive=o.proactive||{},preferences=o.preferences||{},referrals=o.referrals||{},recent=o.recent||{};const cards=[["????????????",users.total??0,`????? ? ????`],["????? ?? 1 ????",users.new_1d??0,`??????????? ?? ?????`],["????? ?? 7 ????",users.new_7d??0,`??????????? ?? ??????`],["Premium",users.premium_total??0,`????????: ${users.active_with_messages??0}`],["??????",users.admins_total??0,`????? env ? ??????`],["?????????",content.messages_total??0,`????? ?? 30?: ${users.new_30d??0}`],["???????",payments.successful_payments??0,`???????: ${payments.revenue??0}`],["AI",`${runtime.queue_size??0}/${runtime.queue_capacity??0}`,`Workers: ${runtime.workers??0}`],["Support",support.users_with_support_profile??0,`panic: ${episodes.panic??0}`],["Proactive",proactive.sent_1d??0,`reply rate: ${proactive.reply_after_proactive_rate??0}%`],["Prefs TZ",preferences.users_with_timezone??0,`opt-out now: ${preferences.proactive_disabled_users??0}`],["????????",referrals.total??0,`?????????: ${referrals.converted??0}`]];$('#overview-cards').innerHTML=cards.map(x=>`<div class="card"><div class="stat-label">${x[0]}</div><div class="stat-value">${x[1]}</div><div class="muted">${x[2]}</div></div>`).join('');$('#recent-users').innerHTML=table(['id','username','first_name','active_mode','is_premium','is_admin','created_at'],recent.users||[]);$('#recent-payments').innerHTML=table(['user_id','amount','currency','status','event_time'],recent.payments||[]);$('#support-summary').innerHTML=`<div class="stack">${metricCards([["??????? ?????????",String(support.users_with_support_profile??0),"???????????? ? support profile"],["????????",String(referrals.total??0),`?????????: ${referrals.converted??0}`],["Proactive users",String(proactive.users_contacted_7d??0),"???? ??? ????? ?? 7 ????"]])}${kvList([["Panic ???????",esc(num(episodes.panic??0))],["Flashback ???????",esc(num(episodes.flashback??0))],["Insomnia ???????",esc(num(episodes.insomnia??0))],["Self-harm ?????",esc(num(support.self_harm_flags??0))],["Proactive sent",esc(num(proactive.sent_total??0))],["Proactive failed",esc(num(proactive.failed_total??0))],["Reply after proactive",esc(`${proactive.reply_after_proactive_total??0} (${proactive.reply_after_proactive_rate??0}%)`)],["Opt-out after proactive",esc(`${proactive.opt_out_after_proactive_total??0} (${proactive.opt_out_after_proactive_rate??0}%)`)],["Users with timezone",esc(num(preferences.users_with_timezone??0))],["Current opt-out users",esc(num(preferences.proactive_disabled_users??0))],["????????? ??????????",esc(support.last_updated_at||'??? ??????')]])}</div>`}
    function renderHealth(){if(!state.health)return;const db=state.health.db||{},redis=state.health.redis||{},ai=state.health.ai_runtime||{};$('#sidebar-health').innerHTML=`${statusPill(db.ok,'DB OK','DB error')} ${statusPill(redis.ok,'Redis OK','Redis error')}`;$('#health-summary').innerHTML=`<div class="stack">${healthSummary(ai)}${kvList([["???? ??????",`${statusPill(db.ok,'??????????','??????')}<div class="muted">${esc(db.detail||'')}</div>`],["Redis",`${statusPill(redis.ok,redis.detail||'OK',redis.detail||'??????')}<div class="muted">${esc(redis.detail||'')}</div>`],["AI ??????",esc(ai.model||ai.openai_model||'?? ???????')],["?????? ????????",esc(String(ai.busy_workers??0))]])}</div>`;$('#full-health').innerHTML=`<div class="stack">${metricCards([["??? ??????",String(ai.cache_hits??0),"????????? ? runtime, ???? ????????"],["??????? AI",`${ai.queue_size??0}/${ai.queue_capacity??0}`,"??????? ????????"],["???????",String(state.health.modes_count??0),"???????? ? ????????"]])}${kvList([["DB ??????",`${statusPill(db.ok,'OK','??????')}<div class="muted">${esc(db.detail||'')}</div>`],["Redis ??????",`${statusPill(redis.ok,'OK','??????')}<div class="muted">${esc(redis.detail||'')}</div>`],["???????? ?????",esc(String(ai.workers??0))],["???????? ??????",esc(String(ai.busy_workers??0))],["???????",esc(`${ai.queue_size??0}/${ai.queue_capacity??0}`)]])}</div>`;$('#config-files').innerHTML=fileTable(state.health.config_files)}
...
');
    ...
    function renderModes(){if(!state.settings||!state.settings.modes||!state.settings.mode_catalog)return;const m=state.settings.modes,c=state.settings.mode_catalog;const keys=Object.keys(c).sort((a,b)=>(c[a].sort_order||0)-(c[b].sort_order||0));const modeScaleLabel=k=>({allow_bold:'?????? ?????',allow_italic:'??????'}[k]||k);$('#modes-container').innerHTML=keys.map(k=>{const meta=c[k]||{},scale=m[k]||{},numericEntries=Object.entries(scale).filter(([,mv])=>typeof mv==='number'),booleanEntries=Object.entries(scale).filter(([,mv])=>typeof mv==='boolean');return `<div class="mode-card"><div class="mode-head"><div><strong>${esc(meta.icon)} ${esc(meta.name)}</strong><div class="muted">${esc(k)}</div></div><span class="badge">${meta.is_premium?'Premium':'Free'}</span></div><div class="three"><label>????????<input data-catalog="${k}.name" value="${esc(meta.name)}"></label><label>??????<input data-catalog="${k}.icon" value="${esc(meta.icon)}"></label><label>???????<input data-catalog="${k}.sort_order" type="number" value="${meta.sort_order??0}"></label></div><label class="checkbox"><input data-catalog="${k}.is_premium" type="checkbox" ${meta.is_premium?'checked':''}>Premium</label><label>????????<textarea data-catalog="${k}.description">${esc(meta.description)}</textarea></label><label>???<input data-catalog="${k}.tone" value="${esc(meta.tone)}"></label><label>????????????? ?????????<input data-catalog="${k}.emotional_state" value="${esc(meta.emotional_state)}"></label><label>???????<textarea data-catalog="${k}.behavior_rules">${esc(meta.behavior_rules)}</textarea></label><label>????? ?????????<textarea data-catalog="${k}.activation_phrase">${esc(meta.activation_phrase)}</textarea></label><div class="three">${numericEntries.map(([mk,mv])=>`<label>${mk}<input data-mode-scale="${k}.${mk}" type="number" min="0" max="10" value="${mv}"></label>`).join('')}</div>${booleanEntries.length?`<div class="two">${booleanEntries.map(([mk,mv])=>`<label class="checkbox"><input data-mode-scale="${k}.${mk}" type="checkbox" ${mv?'checked':''}>${esc(modeScaleLabel(mk))}</label>`).join('')}</div>`:''}</div>`}).join('')}
