...
<body>
        ...
        <div class="cols users-layout">
          ...
          <div class="panel">
            ...
            <div class="template-list" id="user-filter-buttons">
              ...
              <button type="button" class="template-chip" data-user-filter="premium_active">Активные Premium</button>
  ...
  <script>
    ...
    function renderOverview(){
      ...
      const cards=[['Пользователи',num(users.total??0),'Всего в базе'],['Premium',num(users.premium_total??0),`Активных: ${num(users.active_with_messages??0)}`],['Новые за 7 дней',num(users.new_7d??0),`За сутки: ${num(users.new_1d??0)}`],['Выручка',num(payments.revenue??0),`Оплат: ${num(payments.successful_payments??0)}`],['Оплаты 30д',num(funnel30Stages.paid?.users??0),`Продления: ${num(funnel30Stages.renewed?.users??0)}`],['Система',`${runtime.queue_size??0}/${runtime.queue_capacity??0}`,`OpenAI: ${runtime.openai_in_flight_requests??0}/${runtime.openai_configured_limit??0}`]];
      ...
      $('#overview-audience').innerHTML=`<div class="stack">${metricCards([['Всего пользователей',num(users.total??0),'База пользователей'],['Новые за 1 день',num(users.new_1d??0),'Свежие регистрации'],['Новые за 30 дней',num(users.new_30d??0),'Рост за месяц'],['Premium',num(users.premium_total??0),`Админов: ${num(users.admins_total??0)}`]])}${kvList([['Сообщений всего',esc(num(content.messages_total??0))],['Активных premium',esc(num(users.active_with_messages??0))],['Рефералов',esc(num(referrals.total??0))],['Конверсий рефералки',esc(num(referrals.converted??0))]])}</div>`;
      $('#overview-revenue').innerHTML=`<div class="stack">${metricCards([['Успешные оплаты',num(payments.successful_payments??0),'Все провайдеры'],['Виртуальные',num(virtualProvider.successful_payments??0),`Выручка: ${num(virtualProvider.revenue??0)}`],['Telegram',num(telegramProvider.successful_payments??0),`Выручка: ${num(telegramProvider.revenue??0)}`],['Офферы 30д',num(funnel30Stages.offer_shown?.users??0),`Оплаты: ${num(funnel30Stages.paid?.users??0)}`]])}${kvList([['Конверсия оффер -> инвойс (30д)',esc(`${funnel30Conversion.offer_to_invoice_pct??0}%`)],['Конверсия инвойс -> paid (30д)',esc(`${funnel30Conversion.invoice_to_paid_pct??0}%`)],['Конверсия paid -> renewed (30д)',esc(`${funnel30Conversion.paid_to_renewed_pct??0}%`)],['Продления 30д',esc(num(funnel30Stages.renewed?.users??0))]])}</div>`;
      ...
      const recentMonetizationRows=(recent.monetization||[]).map(item=>({'Пользователь':item.user_id??'—','Событие':item.event_name||'—','Trigger':item.offer_trigger||'—','A/B':item.offer_variant||'—','Время':item.created_at||'—'}));
      $('#recent-monetization').innerHTML=`<div class="table-wrap overview-table">${table(['Пользователь','Событие','Trigger','A/B','Время'],recentMonetizationRows)}</div>`;
