...
class ProactiveMessageServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_generate_message_sanitizes_memory_before_model_payload(self):
        ...
        self.assertIn("пользователь на premium", system_prompt)
        self.assertNotIn("user is premium", system_prompt)
        self.assertNotIn("user is free", system_prompt)
        self.assertIn("только на русском языке", system_prompt)
        self.assertNotIn("write one short proactive telegram message", system_prompt)
